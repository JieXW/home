...
- 清理了未被站点使用的脏分类模板
- 支持从新增 Markdown 文件增量导入站点
- 针对新增租房站点支持二次校正标题、Logo、描述和分类
...
为了继续复用后续新增的 Markdown 列表，项目增加了：

- `merge_md_into_site_data.py`

用途：

- 读取新增的 Markdown 文件
- 提取其中的链接
- 与现有 `site-data.js` 去重合并
- 对已有站点执行更新，而不是盲目重复追加
- 合并完成后自动重生成导航页
...
```powershell
python merge_md_into_site_data.py "常用网站2.md"
```

#### 租房与房产站点校正

针对 `常用网站2.md` 中新增的一批租房、房产、民宿与公寓类站点，做了专门校正：
